"""
Продолжение обучения модели фортепиано с существующих весов.
Запуск: python src/train_piano_continue.py
"""
...
from data.midi_dataset_guitar_piano import GuitarPianoDataset
from models.lstm_model import LSTMAccompanimentModel

DEVICE        = torch.device("cpu")
DATA_ROOT     = "data/lmd/lmd_matched"
CKPT_DIR      = "models/guitar_piano_checkpoints"
FINAL_PATH    = "models/guitar_piano_final/model.pt"

EPOCHS        = 50       # дополнительные эпохи
BATCH_SIZE    = 64
LR            = 3e-4     # меньше LR для fine-tuning
SAVE_EVERY    = 10
TRAIN_SAMPLES = 16000    # больше данных
VAL_SAMPLES   = 3000
...
def train():
    ...
    print("Загружаю датасет (гитара → фортепиано)...")
    train_ds = GuitarPianoDataset(DATA_ROOT, split="train", max_files=5000)
    val_ds   = GuitarPianoDataset(DATA_ROOT, split="val",   max_files=1000)
    ...
    # Загружаем уже обученные веса
    if os.path.exists(FINAL_PATH):
        model.load_state_dict(torch.load(FINAL_PATH, map_location=DEVICE, weights_only=False))
        print(f"Загружены веса из {FINAL_PATH}")
    else:
        print("Существующие веса не найдены — обучаю с нуля")
    ...
    print(f"LR: {LR} (пониженный для fine-tuning)")
    best_val = float("inf")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % SAVE_EVERY == 0:
            torch.save({"epoch": epoch, "model": model.state_dict()},
                       os.path.join(CKPT_DIR, f"finetune_epoch_{epoch:03d}.pt"))
